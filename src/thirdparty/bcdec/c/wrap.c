...
// based on MikuMikuLibrary/MikuMikuLibrary.Native/TextureDecoderCore.cpp
// https://github.com/blueskythlikesclouds/MikuMikuLibrary/blob/f0e5ebc72358bd3d18ad26993eade5b2888ed7d8/MikuMikuLibrary.Native/TextureDecoderCore.cpp#L97
// Do note that the colors are still somewhat off, but it's whatever, good enough.
    ...
    int channels, int chroma_channels)
{
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            unsigned char cur_lum = (unsigned char)luminance[(y * w + x) * 2];
            unsigned char cur_chroma_blue = (unsigned char)chroma[(chroma_y * (w / 2) + chroma_x) * 2];
            unsigned char cur_chroma_red = (unsigned char)chroma[(chroma_y * (w / 2) + chroma_x) * 2 + 1];

            float R = cur_lum + 1.5748f * ((cur_chroma_red * 1.003922f) - 128.501895f);
            float G = cur_lum - 0.1873f * ((cur_chroma_blue * 1.003922f) - 128.501895) - 0.4681f * ((cur_chroma_red * 1.003922f) - 128.501895f);
            float B = cur_lum + 1.8556f * ((cur_chroma_blue * 1.003922f) - 128.501895f);

            final_pixels[(y * w + x) * 4 + 0] = (unsigned char)fminf(fmaxf(R, 0), 255);
            final_pixels[(y * w + x) * 4 + 1] = (unsigned char)fminf(fmaxf(G, 0), 255);
            final_pixels[(y * w + x) * 4 + 2] = (unsigned char)fminf(fmaxf(B, 0), 255);
            final_pixels[(y * w + x) * 4 + 3] = (unsigned char)luminance[(y * w + x) * 2 + 1];
        }
    }
...
// same as below, but without the resizing. this is the original version
// char* jnko_decode_ycbcr(char* luminance, char* chroma, int w, int h,
//                         int channels, int chroma_channels) {
//   char* final_pixels = (char*)malloc(w * h * 4 * sizeof(unsigned char));
