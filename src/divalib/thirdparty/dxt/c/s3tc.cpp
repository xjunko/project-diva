// unsigned long PackRGBA(): Helper method that packs RGBA channels into a
// single 4 byte pixel.
//
...
unsigned long PackRGBA(unsigned char r, unsigned char g, unsigned char b,
                       unsigned char a) {
  return ((r << 24) | (g << 16) | (b << 8) | a);
}

// Mine
void UnpackRGBA(unsigned long packed, unsigned char *r, unsigned char *g,
...
//
// unsigned long x:                     x-coordinate of the first pixel in the
// block. unsigned long y:                     y-coordinate of the first pixel
// in the block. unsigned long width:                 width of the texture being
// decompressed. unsigned long height:                height of the texture
// being decompressed. const unsigned char *blockStorage:   pointer to the block
// to decompress. unsigned long *image:                pointer to image where
// the decompressed pixel data should be stored.

void DecompressBlockDXT1(unsigned long x, unsigned long y, unsigned long width,
                         const unsigned char *blockStorage,
                         unsigned long *image) {
  unsigned short color0 = *(const unsigned short *)(blockStorage);
  unsigned short color1 = *(const unsigned short *)(blockStorage + 2);
  ...
  unsigned long code = *(const unsigned short *)(blockStorage + 4);
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (color0 > color1) {
        ...
        switch (positionCode) {
          ...
          case 2:
            finalColor =
                PackRGBA((r0 + r1) / 2, (g0 + g1) / 2, (b0 + b1) / 2, 255);
            break;
          case 3:
...
// unsigned long *image:                pointer to the image where the
// decompressed pixels will be stored.

void BlockDecompressImageDXT1(unsigned long width, unsigned long height,
                              const unsigned char *blockStorage,
                              unsigned long *image) {
...
void DecompressBlockDXT5(unsigned long x, unsigned long y, unsigned long width,
                         const unsigned char *blockStorage,
                         unsigned long *image) {
  unsigned char alpha0 = *(const unsigned char *)(blockStorage);
  unsigned char alpha1 = *(const unsigned char *)(blockStorage + 1);

  const unsigned char *bits = blockStorage + 2;
  unsigned long alphaCode1 =
      bits[2] | (bits[3] << 8) | (bits[4] << 16) | (bits[5] << 24);
  unsigned short alphaCode2 = bits[0] | (bits[1] << 8);

  unsigned short color0 = *(const unsigned short *)(blockStorage + 8);
  unsigned short color1 = *(const unsigned short *)(blockStorage + 10);
  ...
  unsigned long code = *(const unsigned long *)(blockStorage + 12);
...
// void BlockDecompressImageDXT5(): Decompresses all the blocks of a DXT5
// compressed texture and stores the resulting pixels in 'image'.
//
// unsigned long width:                 Texture width.
// unsigned long height:                Texture height.
...
void BlockDecompressImageDXT5(unsigned long width, unsigned long height,
                              const unsigned char *blockStorage,
                              unsigned long *image) {
  unsigned long blockCountX = (width + 3) / 4;
  unsigned long blockCountY = (height + 3) / 4;
